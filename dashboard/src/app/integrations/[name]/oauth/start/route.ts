...
export async function GET(request: NextRequest, context: { params: Promise<{ name: string }> }) {
  ...
  const integrationName = encodeURIComponent(name);
  ...
  const localTarget = new URL(`/agent/v1/integrations/${integrationName}/oauth/start`, origin);
  ...
  fwdHeaders.set("referer", origin + `/integrations/${integrationName}`);
  ...
  const fallback = new URL(`/integrations/${integrationName}`, origin);
