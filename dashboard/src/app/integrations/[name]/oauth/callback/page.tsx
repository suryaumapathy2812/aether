...
export default function PluginOAuthCallbackPage() {
  ...
  const integrationName = params.name as string;
  ...
  useEffect(() => {
    ...
    if (oauthError) {
      router.replace(`/integrations/${integrationName}?error=${encodeURIComponent(oauthError)}`);
      return;
    }

    if (!code) {
      router.replace(`/integrations/${integrationName}?error=${encodeURIComponent("missing_code")}`);
      return;
    }
    ...
    const run = async () => {
      try {
        ...
        if (!res.ok) {
          ...
          router.replace(`/integrations/${integrationName}?error=${encodeURIComponent(msg)}`);
          return;
        }
        router.replace(`/integrations/${integrationName}?connected=true`);
      } catch (error: unknown) {
    ...
    void run();
  }, [session, isPending, router, integrationName, code, state, oauthError]);
