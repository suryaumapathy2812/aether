...
func TestManagerDuplicateRejected(t *testing.T) {
	...
	mustWritePlugin(t, filepath.Join(builtin, "one", "plugin.yaml"), map[string]any{"name": "dup"})
	mustWritePlugin(t, filepath.Join(user, "two", "plugin.yaml"), map[string]any{"name": "dup"})
...
func TestInstallFromSource(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/owner/repo/main/plugin.yaml":
...
func TestDiscoverSyncsPluginsToStateStore(t *testing.T) {
	...
	mustWritePlugin(t, filepath.Join(builtin, "weather", "plugin.yaml"), map[string]any{
		"name":         "weather",
		"display_name": "Weather",
